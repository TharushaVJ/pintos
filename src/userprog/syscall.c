  ...
  void *esp = f->esp;
  	...
  	switch(syscall_no)
    {
      ...
      case SYS_OPEN:
        ...
        {
          struct file_descriptor *fdes = malloc(sizeof(struct file_descriptor));
          fdes->fd = thread_current()->next_fd++;
          fdes->file = f_ptr;
          list_push_back(&thread_current()->file_list, &fdes->elem);
          f->eax = fdes->fd;
        }
        break;
      ...
      case SYS_FILESIZE:
        ...
        int fsize_fd = *(int *)(esp + 4);
        struct file *fsize_file = get_file(&thread_current()->file_list, fsize_fd);
        if(fsize_file == NULL)
        {
          f->eax = -1;
          return;
      ...
      case SYS_WRITE:
        ...
        if(!check_pointer(write_buffer) || !check_pointer(write_buffer + write_size)) 
        {
          thread_exit(-1);
          return;
        }

        if(write_fd == 1) // writing to console
        {
      ...
      case SYS_CLOSE:
        ...
        struct file_descriptor *close_fdes= get_file_descriptor(&thread_current()->file_list, close_fd);
        if(close_fdes == NULL)
        {
          return;
        }
        ...
        {
          acquire_file_lock();
          file_close(close_fdes->file);
          release_file_lock();
          list_remove(&close_fdes->elem);
          free(close_fdes);
        }
        break;
    }
